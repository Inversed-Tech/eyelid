...
fn test_cyclotomic_mul(){
    ...
    xnm1.coeffs = [Fq79::zero(); MAX_POLY_DEGREE].to_vec();
    xnm1.coeffs[MAX_POLY_DEGREE-1] = Fq79::one(); // Xˆ{N-1}, multiplying but it will rotate by N-1 and negate (except the first)
...
pub fn rand_pol() -> DensePolynomial::<Fq79>{
    ...
    DensePolynomial::<Fq79>::rand(MAX_POLY_DEGREE-1, &mut rng)
}
...
pub fn cyclotomic_mul(a: &DensePolynomial::<Fq79>, b: &DensePolynomial::<Fq79>) -> DensePolynomial::<Fq79>{
    ...
    assert!(a.coeffs.len() <= MAX_POLY_DEGREE);
    assert!(b.coeffs.len() <= MAX_POLY_DEGREE);
    for i in 0..MAX_POLY_DEGREE {
        ...
        if i + MAX_POLY_DEGREE < res.coeffs.len() {
            ...
            res[i + MAX_POLY_DEGREE] = Fq79::zero();
        };
    }
    res
}
